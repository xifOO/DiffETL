...
class MessageQualityAssessor:
    ...
    def validate_message(self, message: Union[str, bytes, bytearray, memoryview]) -> ValidationResult:
        ...
        is_conventional = bool(self._commit_pattern.match(message.strip())) if is_not_empty else False
        is_within_length = len(message) <= self._max_length_commit if is_not_empty else False
        has_forbidden_words = bool(self._forbidden_pattern.search(message)) if is_not_empty else False
